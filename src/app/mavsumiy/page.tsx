...
export default async function MavsumiyPage({
  ...
  try {
    const data = await getDramas({ page, limit: 30, category: "seasonal" });
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              {page > 1 && (
                <a href={`/mavsumiy?page=${page - 1}`} className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm hover:bg-white/10 transition">Oldingi</a>
              )}
              <span className="px-4 py-2 text-sm text-gray-400">{page} / {totalPages}</span>
              {page < totalPages && (
                <a href={`/mavsumiy?page=${page + 1}`} className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm hover:bg-white/10 transition">Keyingi</a>
              )}
            </div>
          )}
        </>
