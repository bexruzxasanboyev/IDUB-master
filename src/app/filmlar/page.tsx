...
export default async function FilmlarPage({
  searchParams,
}: {
  ...
  try {
    const data = await getDramas({ page, limit: 30, category: "film" });
    ...
    <div className="container pt-24 sm:pt-28 md:pt-30 pb-10 text-white">
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Hozircha bu bo'limda filmlar mavjud emas. Qayta tashrif buyurib turing!"
        />
      ) : (
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              {page > 1 && (
                <a href={`/filmlar?page=${page - 1}`} className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm hover:bg-white/10 transition">
                  Oldingi
                </a>
              )}
